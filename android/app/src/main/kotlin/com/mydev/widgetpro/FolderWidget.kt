...
class FolderWidget : AppWidgetProvider() {
    companion object {
        fun update(ctx: Context, mgr: AppWidgetManager, id: Int) {
            ...
            val r = prefs.getInt("r", 26); val g = prefs.getInt("g", 26); val b = prefs.getInt("b", 46)
            ...
            val launchIntent = Intent(ctx, FolderWidget::class.java).setAction("com.mydev.widgetpro.FOLDER_LAUNCH").putExtra("wid", id)
    ...
    override fun onAppWidgetOptionsChanged(ctx: Context, mgr: AppWidgetManager, id: Int, opts: Bundle) {
        ...
        ctx.getSharedPreferences("wp_folder", Context.MODE_PRIVATE).edit().putInt("cols_$id", cols).apply()
        update(ctx, mgr, id)
    }
}
